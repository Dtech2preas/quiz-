...
def generate_algebra_questions():
    generator = TopicGenerator(
        topic_name="Patterns and Relationships",
        topic_prefix="ML_PATT",
...
def generate_revision_questions():
    generator = TopicGenerator(
        topic_name="Revision and Examination Preparation",
        topic_prefix="ML_REV",
        subtopics=["Mixed Topics", "Real-life Case Studies", "Data Interpretation"]
    )

    names = ["Kagiso", "Lebo", "Mandla", "Noxolo", "Tshepo", "Buhle", "Thabo", "Sipho"]
    places = ["Cape Town", "Johannesburg", "Durban", "Pretoria"]
    ...
    while not generator.is_done():
        ...
        if subtopic == "Mixed Topics":
            if difficulty == "easy":
                price = random.randint(100, 500)
                vat = price * 0.15
                ans = price + vat
                q = f"In a revision test, {name} is asked to calculate the total cost of an item priced at R{price} before VAT. If VAT is 15%, calculate the total price including VAT."
                ans_str = f"R{ans:.2f}"
                wrong = [f"R{price - vat:.2f}", f"R{price:.2f}", f"R{vat:.2f}", f"R{(price * 1.14):.2f}", f"R{(price * 1.20):.2f}", f"R{price + 15:.2f}", f"R{price - 15:.2f}", f"R{ans + 50:.2f}"]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"VAT = 15% of R{price} = R{vat:.2f}. Total Price = R{price} + R{vat:.2f} = R{ans:.2f}.")
            elif difficulty == "medium":
                ...
                m = int((time - h) * 60)
                ans_str = f"{h} hours and {m} minutes" if m > 0 else f"{h} hours"
                q = f"A combined revision question asks: A taxi travels {dist} km at an average speed of {speed} km/h to {random.choice(places)}. Calculate the total time taken."
                wrong = [f"{dist * speed} hours", f"{dist + speed} hours", f"{speed / dist} hours", f"{h} hours and {m + 10} minutes", f"{h + 1} hours", f"{h} hours and {int(m/2)} minutes", f"{time:.2f} hours", f"{h * 60 + m} hours"]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"Time = Distance ÷ Speed = {dist} ÷ {speed} = {time:.2f} hours. 0.{(time-h)*100:.0f} hours × 60 = {m} minutes. Total = {ans_str}.")
            else:
                length = random.randint(10, 25)
                width = random.randint(5, 15)
                area = length * width
                cost_per_sqm = random.randint(150, 300)
                total_cost = area * cost_per_sqm
                q = f"A school hall is {length} m long and {width} m wide. If new flooring costs R{cost_per_sqm} per square metre, calculate the total cost to tile the hall."
                ans_str = f"R{total_cost}"
                wrong = [f"R{2 * (length + width) * cost_per_sqm}", f"R{area}", f"R{(length + width) * cost_per_sqm}", f"R{total_cost + 1000}", f"R{total_cost - 500}", f"R{area + cost_per_sqm}", f"R{(length * width) * 2}", f"R{total_cost * 2}"]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"Area = {length} × {width} = {area} m². Total Cost = {area} × {cost_per_sqm} = R{total_cost}.")
        elif subtopic == "Real-life Case Studies":
            if difficulty == "easy":
                monthly = random.randint(5000, 15000)
                bonus_pct = random.randint(5, 15)
                bonus = monthly * (bonus_pct / 100)
                ans = monthly + bonus
                q = f"In a case study, {name} earns a basic monthly salary of R{monthly}. They receive a {bonus_pct}% bonus. Calculate their total income for the month."
                ans_str = f"R{int(ans)}"
                wrong = [f"R{int(monthly + bonus_pct)}", f"R{int(monthly - bonus)}", f"R{int(monthly + (bonus*2))}", f"R{int(ans + 1000)}", f"R{int(bonus)}", f"R{int(monthly * bonus_pct)}", f"R{int(monthly / (bonus_pct/100))}", f"R{int(ans - 500)}"]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"Bonus = {bonus_pct}% of R{monthly} = R{int(bonus)}. Total Income = R{monthly} + R{int(bonus)} = R{int(ans)}.")
            elif difficulty == "medium":
                ...
                rent_pct = random.randint(25, 40)
                ans = total_budget * (rent_pct / 100)
                q = f"A municipal case study shows a family with a monthly budget of R{total_budget}. They spend {rent_pct}% on rent. How much is spent on rent?"
                ans_str = f"R{int(ans)}"
                wrong = [f"R{int(total_budget - ans)}", f"R{int(ans * 12)}", f"R{int(total_budget / (rent_pct/100))}", f"R{rent_pct}", f"R{int(total_budget + ans)}", f"R{int(ans / 2)}", f"R{int(ans * 2)}", f"R{int(ans + 500)}"]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"Rent Expense = {rent_pct}% of R{total_budget} = ( {rent_pct} ÷ 100 ) × {total_budget} = R{int(ans)}.")
            else:
                ...
                threshold = 200000
                tax = base_tax + ((taxable - threshold) * (rate / 100))
                q = f"A tax case study requires calculating total tax payable. {name}'s taxable income is R{taxable}. The tax tables state they must pay R{base_tax} plus {rate}% of the amount over R{threshold}. Calculate the total tax payable."
                ans_str = f"R{int(tax)}"
                wrong = [f"R{int(base_tax)}", f"R{int(taxable * (rate/100))}", f"R{int(base_tax + (taxable * (rate/100)))}", f"R{int((taxable - threshold) * (rate/100))}", f"R{int(tax + 5000)}", f"R{int(tax - 5000)}", f"R{int(taxable - tax)}", f"R{int(taxable * 0.15)}"]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"Amount over threshold = R{taxable} - R{threshold} = R{taxable - threshold}. {rate}% of R{taxable - threshold} = R{int((taxable - threshold) * (rate/100))}. Total Tax = R{base_tax} + R{int((taxable - threshold) * (rate/100))} = R{int(tax)}.")
        elif subtopic == "Data Interpretation":
            if difficulty == "easy":
                val1 = random.randint(20, 50)
                val2 = random.randint(60, 100)
                diff = val2 - val1
                q = f"An exam graph question shows {val1} items in Group A and {val2} in Group B. How many more items are in Group B?"
                ans_str = str(diff)
                wrong = [str(val1 + val2), str(val2 * val1), str(val1), str(val2), str(diff + 10), str(diff - 5), str(val2 + 10), str(diff * 2)]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"Difference = Group B - Group A = {val2} - {val1} = {diff}.")
            elif difficulty == "medium":
                ...
                pct = random.choice([10, 20, 25, 30, 40, 50])
                ans = int(total * (pct / 100))
                q = f"A pie chart in a mock exam represents {total} voters. If {pct}% chose Party X, how many people voted for Party X?"
                ans_str = str(ans)
                wrong = [str(pct), str(total - ans), str(total + ans), str(int(total / (pct/100))), str(ans * 2), str(int(ans / 2)), str(ans + 10), str(ans - 10)]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"{pct}% of {total} = ({pct} ÷ 100) × {total} = {ans}.")
            else:
                ...
                avg = sum(nums) / 4
                ans_str = f"{avg:.2f}" if not avg.is_integer() else f"{int(avg)}"
                q = f"An examination question asks to calculate the mean (average) of the dataset: {', '.join(map(str, nums))}."
                wrong = [str(sum(nums)), str(max(nums) - min(nums)), str(sorted(nums)[1]), f"{avg + 5:.2f}", f"{avg - 5:.2f}", f"{(sum(nums)/3):.2f}", f"{(sum(nums)/5):.2f}", str(int(avg*2))]
                generator.add_question(subtopic, difficulty, q, ans_str, wrong, f"Mean = Sum of values ÷ Number of values = {sum(nums)} ÷ 4 = {ans_str}.")
...
if __name__ == "__main__":
    os.makedirs("dataset/grade11/mathematical_literacy", exist_ok=True)

    print("Generating Finance...")
    fin_gen = generate_finance_questions()
    fin_gen.save_to_json("dataset/grade11/mathematical_literacy/paper1_mathlit11_finance.json")

    print("Generating Measurement...")
    meas_gen = generate_measurement_questions()
    meas_gen.save_to_json("dataset/grade11/mathematical_literacy/paper2_mathlit11_measurement.json")

    print("Generating Maps and Plans...")
    maps_gen = generate_maps_questions()
    maps_gen.save_to_json("dataset/grade11/mathematical_literacy/paper2_mathlit11_maps_plans.json")

    print("Generating Data Handling...")
    data_gen = generate_data_questions()
    data_gen.save_to_json("dataset/grade11/mathematical_literacy/paper1_mathlit11_data_handling.json")

    print("Generating Probability...")
    prob_gen = generate_probability_questions()
    prob_gen.save_to_json("dataset/grade11/mathematical_literacy/paper1_mathlit11_probability.json")

    print("Generating Revision...")
    rev_gen = generate_revision_questions()
    rev_gen.save_to_json("dataset/grade11/mathematical_literacy/paper1_mathlit11_revision.json")


    print("Generating Patterns and Relationships...")
    alg_gen = generate_algebra_questions()
    alg_gen.save_to_json("dataset/grade11/mathematical_literacy/paper1_mathlit11_patterns.json")
